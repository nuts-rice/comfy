...
impl Heightmap {
    pub fn new(exp: i32, spread: f32, color_name: &str) -> Heightmap {
        ...
        let default_position = Position { x: 0, y: 0, color: default_color };
        let positions =
        ...
        while resolution >= 1 {
                ...
                .step_by(resolution.try_into().unwrap())
            {
                    ...
                    .step_by(resolution.try_into().unwrap())
                {
                    ...
                    let rng_val = random_range(0., 1.0) as f32;
...
static GEN_METHOD: AtomicRefCell<GenerativeMethod> = AtomicRefCell::new(GenerativeMethod::Midpoint);
